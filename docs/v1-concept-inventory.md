...
| **evidence** | The store of actual run outputs. | `evidence/…` + `issues.jsonl`; `experiment.yaml: collection` | `collection` shape-validated; **values drive review coherence warnings** and are **reflected by `hlab status`**; `status` lists which evidence subdirs are non-empty | `EvidenceCollector`/`EvidenceStore` populate (Auto Mode) | MVP (layout + config takes effect); population stretch |
...
| **Auto Mode** | AHL executes cases against runtimes through connectors. | `experiment.yaml: run.mode=auto`; agent-runtime `connector` | `run.mode` enum validated; Auto connector gate (`local_cli`/`script`); Auto state-policy note | `AutoRunner` (Auto phase) | MVP (config); `hlab run` is an explicit exit-2 stub now |
