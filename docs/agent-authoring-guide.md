...
> 给**外层 coding agent**(Claude Code / Cursor / Codex 等)读的。你正在帮用户 author 一个 HDL 实验。HDL 不会自己想实验 —— 那是你的活。

## Your role(你的角色)

你是 **external Designer**。HDL 是 **protocol + validator + reviewer + runner**。

具体说:

- 你据 `goal.md` 和 `experiments/<id>/brief.md` 起草整套实验文件。
- HDL 不调模型自己起草。它只 scaffold、validate、review、run、score、compare。
...
| `docs/file-formats.md` | v1 文件格式权威定义(`program.md` / `rubric.md` / `测试集/` / `versions/` / `connect.md` / `模拟器.md`)。 |
...
- `versions/V1.md`、`versions/V2.md`(……)—— 被对比的版本。**恰好一个**标 `基线: 是`(对基线模式),其余 `基线: 否`。每个版本「这是什么」段写清和基线的差异。
- `测试集/D-01.md`(……)—— 测试 case,一个 case 一个文件。frontmatter 至少含 `id`,body 必须有 `## 起始输入`。
- `rubric.md` —— 评分维度 + 权重,**权重之和 = 1.0**(或百分数 = 100)。
- `模拟器.md` —— 模拟模式下扮用户的 agent(人设 / 背景知识 / 追问策略)。

格式严格按 `docs/file-formats.md`。HDL 的 parser 会校验;过不了 `hdl review` 会标错,过不了 `hdl run` 的 preflight 会硬退。

## Invariants(不变量)

- **V1 / V2 在 `program.md` 汇合。** V1 是人手写 `program.md`,V2 是你据 `brief.md` 写 `program.md`。从 `program.md` 往后(versions / 测试集 / rubric / run / score / compare),两条路完全同构。
...
## Checklist before review(起草完跑 `hdl review` 前自查)

- `program.md`:`假设` / `声明`(环境 / 对话模式 / 状态 / 评分 / 运行模式 / 对比方式)/ `留/丢规则` / `喊人规则` 都填了。
- `versions/`:至少 V1 + V2;每个版本「这是什么」段写清楚和基线的差异。
- 当 `对比方式: 对基线` 时,**恰好一个**版本标 `基线: 是`(线性迭代不要求基线)。
- `测试集/`:每个 case 有 `id`(frontmatter)和 `## 起始输入` 段。
- `rubric.md`:维度齐 + 权重之和 = 1.0(或百分数和 = 100)。
- **brief 的「不能牺牲什么」已经落进 rubric 或 program。**
- 跑 `hdl review <name>` 看 review.md,没有「未起草」即可走 `hdl run`。
...
hdl review <name>     出 review.md(宽松:缺什么标「未起草」)。
                      你可以多次跑,看 authoring 进度。
hdl run <name>        preflight 是硬关卡:program / 版本 / case / 基线数
                      任一不过都拦下。
hdl score <name>      按 rubric 给最近一次 run 的对话打分。
