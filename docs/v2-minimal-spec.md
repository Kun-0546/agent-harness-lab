...
`program.md` 是中枢,不是垃圾桶。进 program 的:实验假设、对话 / 运行 / 对比方式、留-丢规则、喊人规则、红线摘要。不进 program 的各有各的位置:provenance → 各文件 frontmatter;审核全文 → review.md;golden case → calibration/golden/;Designer 的推理过程 → 不落盘;authority matrix 复杂配置 → 以后单独配。
...
- **brief.md 已填、还没起草** → 调 Designer(§4)起草 program.md / versions/ / 测试集/ / rubric.md / 模拟器.md,每个文件带 provenance 头(§5),再生成 review.md(§6)。提示:「草案生成,审 review.md」。
...
- **失败硬退**:模型调用失败、或输出解析不出来 → 抛 WorkflowError、清掉半成品,不留残包。
...
每个文件在 frontmatter 带一个来源戳。本刀只要两字段:

```
---
provenance:
  source: agent_drafted        # 或 human
  approval_state: pending      # 或 approved
---
```

- `hdl draft` 生成的文件:`source: agent_drafted`、`approval_state: pending`。
- 人手写的(brief.md / goal.md / connect.md):`source: human`(可不写,缺省即 human)。
- `run` 跑前:若有 `agent_drafted` 且 `pending` 的文件,打印一行提醒(不硬拦 —— 硬拦留 v2.5)。
- 翻成 `approved` 的机制(`hdl approve`)留到 v2.5;本刀只记录、只提醒。
...
sentinel cases、adversarial bad versions、calibration 校验闭环(拿 golden / 坏版本测 judge)、judge regression、`hdl approve` 审批闭环、run 前硬拦未批准产物、authority matrix 可编辑 + mode 选择器、agent-operated loop、自迭代、escalation 规则 —— 对应 v0.4.1 §9 的 v2.5 / v3 / v4。
...
6. provenance 头 + `hdl draft` 把它写进生成的文件。
7. review.md 生成。
